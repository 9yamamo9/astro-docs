...
サイト内の別のページへリンクを張るには、HTML標準の[`<a>`要素](https://developer.mozilla.org/ja/docs/Web/HTML/Element/a)をコンポーネントテンプレートに記述してください。ここでは、相対ファイルパスではなく、ルートドメインからの相対URLパスをリンクとして使用してください。

次の例は、`example.com`の他のページから`https://example.com/authors/sonali/`へのリンクの場合を表しています。

```astro title="src/pages/index.astro"
<a href="/authors/sonali/">Sonaliについて</a>さらに詳しくみる。
```
...
ページは完全なHTMLドキュメントを作成しなければなりません。もし明示的に含まれていない場合、Astroはデフォルトで`src/pages`内にある`.astro`コンポーネントに、必要となる`<!DOCTYPE html>`宣言と`<head>`を追加します。
